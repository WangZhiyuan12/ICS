...
- mov指令只会改变特定的低位的数字，不会影响到高位。
- 程序中的栈顶是向下开的
- push %ebp:先将rsp-8,再将ebp中存的内容推入到rsp所在的地址处。
- 我们可以通过寻址的方式在栈中寻找相关的数据比如4(%rsp)
- pushq %rsp是存储的是老的数字。
- 排除掉未运行分配站的空间，以及回收栈的空间的代码为过程体。
- 函数体传入的参数是在rsp上方的某个位置。
- c语言中的指针的本质上是个地址。
- 通常指针是放在寄存器中，通过寄存器来访问存储器上的特定位置的值。局部变量放于寄存器中。
- lea:形式上是从存储器读数据到寄存器，实际上试讲存储器的地址读入到寄存器中。
- 一元操作，数据既可以作为源，也作目的
- 二元操作，第二个操作数是作为目的，也作为源。
- 移位操作，操作数其中之一，要嘛是立即数，要么是寄存器
- 除了右移操作之外，我们都不考虑到有符号还是无符号。
- 特殊操作比如32bit32bit相乘的话，就需要其中有一个数存储在$eax中，然后分为高32bit和低位32bit之间的相乘。此时的寄存器入栈的顺序为%edx和%eax
- div执行的是无法很好位的除法，会先将%edx设置成零。
# 复习3.6
## 单词
## 知识点
- 控制语句，可以控制跳转到函数的特定的位置。
- CF:无符号数中的最高位师傅溢出
- ZF：最近操作的结果是否是零
- SF：最近的操作数是负数
- OF：最近的操作使补码溢出，正溢出和负溢出
- lea不会改变任何条件码
- 会设置条件码，在图3.7中。xorl，CF和OF会设置为0.对于移位操作，进位标志将设置为最后溢出的那个位，溢出标志设置为0
- cmp 和test也会设置为条件码，如果两个操作使相同的话，cmp将会ZF=1.
- test通常改变零和负数位，来检验是否是零还是正数。
- set会将某一个位标记成0或者1，通过的是movzbl实现扩位。
- a-b=t,当t为很大的正数时，正溢出t<0，当t很小的时候，负溢出，t>0。
- cmpl指令会在a<b的过程中标志进位数。
- jmp * %eax
- 出现 align语句中目的是为了优化地使用高速缓存存储器
- 在汇编中通常使用pc实现相对的寻址。
- 程序计数器不是记住绝对地址，而是相对偏移量
- 跳转表的效率开关的数量和开关情况值的稀少程度，值的跨度药效，跳转表是一个数组，用索引来表示为值。
- 开关数量较多，跨度较小的时候偶，就会使用跳转表。
- .long .L4跳转表的申明。
- .rodata 只读数据
# 复习3.7
## 单词
## 知识点
- 过程：包括过程参数和返回值，控制从一段代码跳转到另外一段的，还有进入过程中局部变量分配空间，退出的时候释放这些空间
- 通过程序栈来实现。传递参数，存储返回信息，保存寄存器，本地的存储。
- 栈帧：有两个指针在最顶端，%ebp作为帧指针，%esp栈指针
- 访问信息一般通过%ebp
- 调用者在调用被调用者时，会将在调用者中的返回地址压入陪调用者中，返回地址。
- 栈帧通过从ebp开始后面保存其他的寄存器的值
- 如果寄存器不够放的话，可以使用栈来保存数据（结构体或者是其他
- 向低地址处扩展，%esp指向的栈顶。
- call会将返回值入栈，同时将指针调到被调用过程的起始处
- ret指令将会跳转到返回地址处
- leave语句中相当于将栈指针指向栈底，然后将%ebp恢复，之后esp指针指向地址。
- 程序寄存器是唯一一个呗所有过程共享的资源，这时，应该要防止在被调函数中覆盖掉这些数据。
- 呗调用者保存的意思是将一些寄存器，在被调用函数覆盖他们之前，必须推入栈中，然后，在ret之前，恢复他们。
